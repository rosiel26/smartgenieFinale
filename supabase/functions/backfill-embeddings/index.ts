...
/**
 * Backfill Embeddings - Self-hosted, $0 Cost
 * Generates embeddings for all dishes missing them
 */
...
// Local embedding computation (same as generate-embedding function)
...
async function fetchImageBytes(url: string): Promise<Uint8Array> {
  const resp = await fetch(url, { 
    headers: { 'User-Agent': 'SmartGenie-Backfill/1.0' }
  });
  if (!resp.ok) throw new Error(`Failed to fetch: ${resp.status}`);
  return new Uint8Array(await resp.arrayBuffer());
}

async function backfillBatch(offset: number, limit: number): Promise<{ processed: number; errors: string[] }> {
  ...
  if (!rows || rows.length === 0) return { processed: 0, errors: [] };
  ...
  for (const row of rows) {
    try {
      ...
      const imageBytes = await fetchImageBytes(row.image_url);
      ...
      processed++;
      console.log(`✓ ${row.name} (${processed})`);
    } catch (e) {
...
Deno.serve(async (req) => {
  ...
  try {
    ...
    // Get count of dishes needing embeddings
    ...
    console.log(`Starting backfill: ${count} dishes need embeddings`);

    let totalProcessed = 0;
    const allErrors: string[] = [];

    for (let batch = 0; batch < maxBatches; batch++) {
      const { processed, errors } = await backfillBatch(0, batchSize); // Always start from 0 since we filter by null
      totalProcessed += processed;
      allErrors.push(...errors);

      if (processed === 0) {
        console.log('No more dishes to process');
        break;
      }

      console.log(`Batch ${batch + 1}: processed ${processed} dishes`);
    }
    ...
    return new Response(
      JSON.stringify({
        success: true,
        totalProcessed,
        remaining: (count || 0) - totalProcessed,
        errors: allErrors.slice(0, 10), // Only return first 10 errors
